"""SQLAlchemy models for MeetMind — SQLite compatible."""
...
class Organization(Base):
    ...
    settings_data: Mapped[Optional[str]] = mapped_column(Text, default="{}")
...
class User(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    integrations_data: Mapped[Optional[str]] = mapped_column(Text, default="{}")
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class Meeting(Base):
    ...
    participants_data: Mapped[Optional[str]] = mapped_column(Text, default="[]")
    extra_data: Mapped[Optional[str]] = mapped_column(Text, default="{}")
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    ...
    report: Mapped[Optional["MeetingReport"]] = relationship(
...
class MeetingReport(Base):
    ...
    decisions_data: Mapped[Optional[str]] = mapped_column(Text, default="[]")
    open_questions_data: Mapped[Optional[str]] = mapped_column(Text, default="[]")
    risks_data: Mapped[Optional[str]] = mapped_column(Text, default="[]")
    key_insights_data: Mapped[Optional[str]] = mapped_column(Text, default="[]")
    follow_ups_data: Mapped[Optional[str]] = mapped_column(Text, default="[]")
...
class ActionItemModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint("fingerprint", "org_id", name="uq_fingerprint_org"),
    )
    ...
    dependencies_data: Mapped[Optional[str]] = mapped_column(Text, default="[]")
    status: Mapped[str] = mapped_column(String(20), default="pending")
    external_refs_data: Mapped[Optional[str]] = mapped_column(Text, default="{}")
...
class AuditLog(Base):
    ...
    tool_calls_data: Mapped[Optional[str]] = mapped_column(Text, default="[]")
...
class ExecutionResultModel(Base):
    ...
    request_payload_data: Mapped[Optional[str]] = mapped_column(Text)
    response_payload_data: Mapped[Optional[str]] = mapped_column(Text)
